...
def run():
    ...
    for filename in files:
        ...
        if not text:
            print(f"Raw OCR text for {chapter_id} not found! Skipping.")
            continue
            
        # Optional truncating if text is absurdly large, but Gemini 2.5 Pro can handle 1M tokens easily.
...
"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"Error parsing JSON for {chapter_id} on attempt {attempt + 1}:", e)
